...
static int send_message_to_client(char *dataSend, int len)
{
    int result = -1;
    int timeout = 0;
...
static int receive_message_from_client(char *dataReceive)
{
    int result;
    // Create msg_receive
    msg = kmalloc(sizeof(struct msghdr), GFP_KERNEL);
...
static int __init init_socket(void)
{
    ...
    if (opSendFirst == 1)
    {
        if (opSend == 1)
        {
            send_message_to_client(dataSend, sendDataLen);
        }
        if (opRecv == 1)
        {
            receive_message_from_client(dataReceive);
        }
    }
    else
    {
