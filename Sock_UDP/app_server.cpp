...
int recv_msg_from_client(int socketfd, char *dataReceive)
{
  int s;
...
int send_msg_to_client(int socketfd, char *dataSend)
{
  int s;
  int timeout;
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < argc; i++)
  {
    ...
    {
      recv_msg_from_client(fd, dataReceive);
    }

    if (strcmp(argv[i], "-w") == 0)
    {
      send_msg_to_client(fd, argv[i + 1]);
    }
  }
